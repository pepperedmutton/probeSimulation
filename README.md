...
- `periodic_x`: optionally wrap the Poisson solve along x to mimic an infinitely wide plate.
- `enforce_lateral_uniformity`: when `true`, charge and potential are averaged along x before streaming so the rendered 2D slice becomes a horizontal sheath band (useful when you want the visual to emphasize the planar assumption even though the domain is wider).
